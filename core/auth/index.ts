...
async function loginWithPassword(credentials: unknown): Promise<User | null> {
  ...
  // Post-login cleanup: cart restoration toast + anonymous session purge.
  // PM-CUSTOMIZATION: wrapped in try/catch so failures here (e.g. next-intl
  // `getTranslations` having no locale context when sign-in is triggered
  // from outside the `[locale]` segment, like `/dev/preview/account/*`)
  // don't break the login itself. Without this, the BC login would succeed
  // but the throw would bubble up as CallbackRouteError and the user would
  // see "couldn't reach our authentication service" despite valid creds.
  try {
    await handleLoginCart(cartId, result.cart?.entityId);
    await clearAnonymousSession();
  } catch (cleanupError) {
    // eslint-disable-next-line no-console
    console.error('[loginWithPassword] post-login cleanup failed (login still succeeded):', cleanupError);
  }
...
async function loginWithJwt(credentials: unknown): Promise<User | null> {
  ...
  // PM-CUSTOMIZATION: same try/catch as loginWithPassword — see that
  // function's comment for why the cleanup can throw (no-locale calls).
  try {
    await handleLoginCart(cartId, result.cart?.entityId);
    await clearAnonymousSession();
  } catch (cleanupError) {
    // eslint-disable-next-line no-console
    console.error('[loginWithJwt] post-login cleanup failed (login still succeeded):', cleanupError);
  }
